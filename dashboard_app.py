...
def plot_line_chart(df, x_col, y_col, title, ax, color_key="dark_green"):
    ...
    if df[y_col].nunique() <= 1 and len(df) > 0:
        ...
        ax.set_ylim(value * 0.95 if value > 0 else value - 1, 
                    value * 1.05 if value > 0 else value + 1)
    
    ax.plot(df[x_col], df[y_col], marker='o', linestyle='-', color=HEINEKEN_COLORS[color_key])
    ax.set_title(title, fontsize=14, fontweight='bold', color=HEINEKEN_COLORS["black"])
    ax.set_xlabel(x_col.replace('_', ' ').title(), fontsize=12)
    ax.set_ylabel(y_col.replace('_', ' ').title(), fontsize=12)
    
    # Apply date format and rotation for X-axis
    if pd.api.types.is_datetime64_any_dtype(df[x_col]):
        ax.xaxis.set_major_formatter(FuncFormatter(lambda x, pos: pd.to_datetime(x).strftime('%Y-%m-%d') if x >= 1000 else ''))
...
def plot_correlation_chart(df, x_col, y_col, title, ax):
    ...
    if not temp_df.empty:
        ax.scatter(temp_df[x_col], temp_df[y_col], color=HEINEKEN_COLORS["red"], alpha=0.6)
        corr = temp_df.corr().iloc[0, 1]
    else:
        corr = 0.0

    ax.set_title(f"{title} (Corr: {corr:.2f})", fontsize=14, fontweight='bold', color=HEINEKEN_COLORS["black"])
    ax.set_xlabel(x_col.replace('_', ' ').title(), fontsize=12)
    ax.set_ylabel(y_col.replace('_', ' ').title(), fontsize=12)
    ax.tick_params(axis='x', rotation=45, labelsize=10)
    ax.grid(False)
...
def main_dashboard():
    ...
    ssi_change = ssi_df['SSI_Increase'].iloc[-1] if 'SSI_Increase' in ssi_df.columns else 0
    ...
    if not interactions_df.empty and not interactions_df.empty:
        ...
        daily_summary = interactions_df.groupby('date').apply(lambda x: pd.Series({
        ...
        if not daily_summary.empty and daily_summary['Total_Attempts'].iloc[-1] > 0:
            last_conn_rate = daily_summary['Connected'].iloc[-1] / daily_summary['Total_Attempts'].iloc[-1] * 100
    ...
    plot_line_chart(ssi_df, 'Date', 'Total_SSI', 'SSI Total ao Longo do Tempo', axes[0, 0], "dark_green")
    
    # Plot 2: Correlation Plot (ROW 0, COL 1)
    plot_correlation_chart(ssi_df, 'Connection_Limit', 'Total_SSI', 'Total SSI vs. Limite de Conexão', axes[0, 1])
    ...
    for i, comp in enumerate(ssi_components):
        ...
        plot_line_chart(ssi_df, 'Date', comp, f'Componente SSI: {comp}', axes[i+1, 0], "lime_green")
        
        # Correlation (Right Column, starting row 1)
        plot_correlation_chart(ssi_df, comp, 'Total_SSI', f'Total SSI vs. {comp}', axes[i+1, 1])
    
    # Aumentando hspace para evitar sobreposição de gráficos adjacentes
    plt.subplots_adjust(top=0.95, bottom=0.05, left=0.05, right=0.95, hspace=0.6, wspace=0.2)
    st.pyplot(fig)
    st.markdown("---")
    ...
    interaction_cols = ['Connection_Limit', 'Follow_Limit', 'Profiles_To_Scan', 'Feed_Posts_Limit']
    ...
    for i, col in enumerate(interaction_cols):
        ...
        plot_line_chart(ssi_df, 'Date', col, f'Limite de Ação: {col.replace("_", " ").title()}', axes[i, 0], "medium_dark_green")
        
        # Plots 15-18: Correlation of Limits with SSI Total
        plot_correlation_chart(ssi_df, col, 'Total_SSI', f'Total SSI vs. {col.replace("_", " ").title()}', axes[i, 1])
    ...
    prob_cols = ['Group_Like_Prob', 'Group_Comment_Prob', 'Feed_Like_Prob', 'Feed_Comment_Prob']
    
    fig, axes = plt.subplots(3, 2, figsize=(18, 15))
    fig.patch.set_facecolor(HEINEKEN_COLORS["off_white"])

    # Plots 19-20: Group Probabilities
    plot_line_chart(ssi_df, 'Date', 'Group_Like_Prob', 'Prob. de Like em Grupos', axes[0, 0], "lime_green")
    plot_line_chart(ssi_df, 'Date', 'Group_Comment_Prob', 'Prob. de Comentário em Grupos', axes[0, 1], "red")

    # Plots 21-22: Feed Probabilities
    plot_line_chart(ssi_df, 'Date', 'Feed_Like_Prob', 'Prob. de Like no Feed', axes[1, 0], "medium_dark_green")
    plot_line_chart(ssi_df, 'Date', 'Feed_Comment_Prob', 'Prob. de Comentário no Feed', axes[1, 1], "dark_green")

    # Plots 23-24: Correlation
    plot_correlation_chart(ssi_df, 'Group_Comment_Prob', 'SSI_Increase', 'Aumento SSI vs. Prob. Comentário em Grupo', axes[2, 0])
    plot_correlation_chart(ssi_df, 'Feed_Like_Prob', 'SSI_Increase', 'Aumento SSI vs. Prob. Like no Feed', axes[2, 1])
    ...
    if not interactions_df.empty:
        ...
        daily_interactions = interactions_df.groupby('date').apply(lambda x: pd.Series({
            'total_attempts': len(x),
            'connected': len(x[x['status'] == 'Connected']),
        ...
        daily_interactions['Conversion_Rate'] = (daily_interactions['connected'] / daily_interactions['total_attempts']) * 100
        
        # Merge with SSI data based on date
        daily_interactions['date'] = pd.to_datetime(daily_interactions['date'])
        merged_df = pd.merge(daily_interactions, ssi_df[['Date', 'Total_SSI']], left_on='date', right_on='Date', how='left')
        
        fig, axes = plt.subplots(5, 2, figsize=(18, 25))
        fig.patch.set_facecolor(HEINEKEN_COLORS["off_white"])
        
        # Plots 25-27: Conversion and Attempts
        plot_line_chart(daily_interactions, 'date', 'total_attempts', 'Tentativas Totais de Perfil por Dia', axes[0, 0], "dark_green")
        plot_line_chart(daily_interactions, 'date', 'connected', 'Conexões Enviadas por Dia', axes[0, 1], "lime_green")
        plot_line_chart(daily_interactions, 'date', 'Conversion_Rate', 'Taxa de Conversão Diária (%)', axes[1, 0], "red")
        
        # Plot 28: Connection vs SSI
        plot_correlation_chart(merged_df.dropna(subset=['Total_SSI']), 'connected', 'Total_SSI', 'Conexões Enviadas vs. SSI Total', axes[1, 1])
        
        # Plots 29-30: Source Analysis (Bar Chart)
        source_counts = interactions_df['source'].value_counts()
        plot_bar_chart(source_counts.reset_index(name='count'), 'source', 'count', 'Interações por Fonte', axes[2, 0], "dark_green")
        
        # Plots 31-32: Target Role Analysis (Top 5)
        top_roles = interactions_df['headline'].str.split(',').str[0].str.strip().value_counts().nlargest(5).reset_index(name='count')
        plot_bar_chart(top_roles, 'headline', 'count', 'Top 5 Manchetes Alvo', axes[2, 1], "medium_dark_green")

        # Plots 33-34: Analytics Dashboard Data
        if not analytics_df.empty:
            plot_line_chart(analytics_df, 'timestamp', 'profile_views', 'Visualizações de Perfil (Dashboard)', axes[3, 0], "lime_green")
            plot_line_chart(analytics_df, 'timestamp', 'post_impressions', 'Impressões de Posts (Dashboard)', axes[3, 1], "dark_green")
        else:
            axes[3, 0].set_title("N/A: Profile Analytics data missing", fontsize=14, fontweight='bold')
            axes[3, 1].set_title("N/A: Profile Analytics data missing", fontsize=14, fontweight='bold')
            axes[3, 0].grid(False); axes[3, 1].grid(False) 
        ...
        if not analytics_df.empty and not ssi_df.empty:
            ...
            daily_analytics = analytics_df.groupby('date').agg({'profile_views': 'max', 'post_impressions': 'max'}).reset_index()
            daily_analytics['date'] = pd.to_datetime(daily_analytics['date'])
            merged_analytics_ssi = pd.merge(daily_analytics, ssi_df[['Date', 'Total_SSI', 'SSI_Increase']], left_on='date', right_on='Date', how='inner')
            
            plot_correlation_chart(merged_analytics_ssi, 'profile_views', 'Total_SSI', 'Total SSI vs. Visualizações de Perfil', axes[4, 0])
            plot_correlation_chart(merged_analytics_ssi, 'post_impressions', 'SSI_Increase', 'Aumento SSI vs. Impressões de Posts', axes[4, 1])
        else:
            axes[4, 0].set_title("N/A: Correlation Analytics missing data", fontsize=14, fontweight='bold')
            axes[4, 1].set_title("N/A: Correlation Analytics missing data", fontsize=14, fontweight='bold')
            axes[4, 0].grid(False); axes[4, 1].grid(False) 
            
        plt.subplots_adjust(top=0.95, bottom=0.05, left=0.05, right=0.95, hspace=0.6, wspace=0.2)
        st.pyplot(fig)
        
    else:
        st.info("Nenhuma interação de perfil registrada no banco de dados SQLite ainda.")
